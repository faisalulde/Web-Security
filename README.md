# Web Security

A personal knowledge base covering web application security concepts, techniques, payloads, and testing methodologies.

This repository grows with my career — content comes from PortSwigger Web Security Academy, CTF competitions, HackerOne disclosed reports, OWASP, and independent research.

---

## Contents

| Topic | Status |
|---|---|
| [Access Control](Access-Control/access-control.md) | Complete |
| [Authentication](Authentication/authentication.md) | Complete |
| SQL Injection | Coming Soon |
| Business Logic | Coming Soon |
| XSS | Coming Soon |
| SSRF | Coming Soon |
| SSTI | Coming Soon |

---

## Structure

Each topic folder contains:

- Concepts and definitions
- Attack techniques and methodologies
- Payloads and examples
- Prevention methods
- Notes from practical testing

---

## Related

- [PortSwigger-Web-Security-Academy](https://github.com/faisalulde/PortSwigger-Web-Security-Academy) - lab progress and exploit scripts
- [security-writeups](https://github.com/faisalulde/security-writeups) - CTF writeups applying these techniques

---

## Connect

- [LinkedIn](https://linkedin.com/in/faisal-ulde)
- [GitHub](https://github.com/faisalulde)
